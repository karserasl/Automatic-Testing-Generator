...
# @Date:   30/11/2020 21:09

import re
import sys

PARAM_OR_RETURNS_REGEX = re.compile(":(?:param|returns|eq)")
RETURNS_REGEX = re.compile(":returns: (?P<doc>.*)", re.S)
RETURNS_EQ = re.compile(":eq: (?P<doc>.*)", re.S)
    ...
    if docstring:
        ...
        if len(lines) > 1:
            ...
            if params_returns_desc:
                ...
                match = RETURNS_EQ.search(params_returns_desc)
                if match:
                    eq = reindent(match.group("doc"))
